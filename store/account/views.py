...
def checkbarcode(request, barcode):
    try:
        barcode = Queuedb.objects.filter(barcode=barcode).update(status='완료', updatetime=datetime.datetime.now())
...
def cancelbarcode(request, barcode):
    try:
        barcode = Queuedb.objects.filter(barcode=barcode).update(status='취소', updatetime=datetime.datetime.now())
        # barcode = Queuedb.objects.get(barcode=barcode)
        # barcode.status = '취소'
        # barcode.save()
